...
func TestMarketPredicates(t *testing.T) {
	...
	t.Run("deal state array predicate", func(t *testing.T) {
		...
		require.Equal(t, changedDeals.Added[0].ID, abi.DealID(3))
		require.Equal(t, changedDeals.Added[0].Deal, *newDeal3)

		require.Len(t, changedDeals.Removed, 0)

		require.Len(t, changedDeals.Modified, 2)
		require.Equal(t, changedDeals.Modified[0].ID, abi.DealID(1))
		require.Equal(t, changedDeals.Modified[0].From, *oldDeal1)
		require.Equal(t, changedDeals.Modified[0].To, *newDeal1)

		require.Equal(t, changedDeals.Modified[1].ID, abi.DealID(2))
		require.Equal(t, changedDeals.Modified[1].From, *oldDeal2)
		require.Equal(t, changedDeals.Modified[1].To, *newDeal2)
	})
	...
	t.Run("deal proposal array predicate", func(t *testing.T) {
		...
		require.Equal(t, changedProps.Added[0].ID, abi.DealID(3))
		require.Equal(t, changedProps.Added[0].Proposal, *newProp3)

		// proposals cannot be modified -- no modified testing

		require.Len(t, changedProps.Removed, 1)
		require.Equal(t, changedProps.Removed[0].ID, abi.DealID(2))
		require.Equal(t, changedProps.Removed[0].Proposal, *oldProp2)
	})
}
